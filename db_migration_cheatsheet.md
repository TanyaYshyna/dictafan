...
### `.env` (ОБЯЗАТЕЛЬНО для alembic)

В этом проекте Alembic берёт строку подключения из переменной окружения `DATABASE_URL` (см. `migrations/env.py`).
...
## 3) Миграции в этом проекте

В репо есть два “канала” миграций:

### Если хочешь «жить без Alembic» (упрощённый режим)

Можно. Тогда правило простое:
...
Что ты теряешь без Alembic:

- автоматический учёт того, *какая миграция уже применена* (таблица `alembic_version`)
- “историю версий” и простой откат

Но для твоего кейса (две базы и ты хочешь руками контролировать) это часто ок.
