...
def recalc_history_current_all_users():
    ...
    3) Пересоздаёт history_current, считая количество завершённых проходов
       (строк с successes > 0) по всем пользователям — это количество проходов
       (медаль 🥇) для каждого упражнения, совпадающее с номерами колонок отчёта.
    """
    try:
        current_email = get_jwt_identity()
        user = get_user_by_email(current_email)
