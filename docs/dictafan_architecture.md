...
- `6000` — выпадающие меню внутри `#book-view-modal` (`.book-actions-menu`, `.section-actions-menu`, задаётся через `.dropdown-menu-wrapper` в `static/css/book_modal.css`) — действуют внутри stacking context модалки книги (`z-index: 100220`) и должны быть выше карточек диктантов из списка (`z-index: 50`)
